...
import {
	getSessionsForUser,
	getSession as getRegistrySession,
...
app.post(
	...
	async (req, res) => {
		...
		if (
			registrySession?.status === "streaming" &&
			...
			if (sessionFile) {
				...
				if (isCompletedTurn && turnState.timestamp) {
					const timeSinceLastMessage =
						Date.now() - turnState.timestamp.getTime();
		...
		if (streaming || hasBuffer) {
			...
			logger.info("Session attach: streaming (RPC)", {
				sessionId,
				username,
				replayed,
				actualStatus,
		...
		try {
			...
			if (cliResult.watching) {
				logger.info(`Session attach: ${cliResult.status} (CLI file watcher)`, {
					sessionId,
					username,
				});
				return res.json({
			...
			logger.warn("Session attach: CLI detection failed", {
				sessionId,
				error: err.message,
			});
		}
...
app.get("/api/events", (req, res) => {
	...
	res.write(
		`data: ${JSON.stringify({ type: "state-snapshot", sessions: userSessions })}\n\n`,
	);
...
async function gracefulShutdown(signal) {
	...
	try {
		...
		logger.warn("SDK shutdown did not complete cleanly", {
			error: err.message,
		});
	}
	...
	try {
		...
		logger.warn("Server close did not complete cleanly", {
			error: err.message,
		});
	}
